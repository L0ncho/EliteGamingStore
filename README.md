...
Hasta la Semana 3 el layout dependía de HTML propio más la hoja `Alonso_PFY2201_CSS_Semana2.css` (entonces en la raíz del proyecto): variables en `:root`, Flexbox en `.menu-principal` y `.redes-sociales`, CSS Grid (`auto-fit` / `minmax`) en `.lista-productos` y media queries a 768px y 480px. Las páginas enlazaban esa hoja así:
...
| --------------------------------------------------------- | --------------------------------------------------------------------- |
| CSS propio en la raíz del proyecto                        | Bootstrap 5.3 por CDN (CSS + JS)                                      |
...
La hoja `Alonso_PFY2201_CSS_Semana2.css` **no se borra** (sigue siendo la entrega de Semana 2), pero **ya no se vincula** y vive en `css/`, fuera de la raíz del sitio. Un prototipo de una sola página (`Alonso_PFY2201_Bootstrap_Semana4.html`) sirvió de ensayo y se movió a `Planes_Fronted` en el escritorio; el sitio real permanece en dos archivos HTML.
...
Las imágenes se referencian con rutas relativas (`images/juego1.jpg`). El CSS de Semanas 2–3 quedó archivado en `css/` y no se carga. `js/` sigue reservada; el estilo y el comportamiento actuales llegan por el CDN de Bootstrap.
...
| `css/Alonso_PFY2201_CSS_Semana2.css` | Hoja histórica de Semanas 2–3, archivada en `css/`. Ya no forma parte del render actual.             |
...
- **Semana 2:** hoja `css/Alonso_PFY2201_CSS_Semana2.css` con variables, modelo de cajas y selectores avanzados. Informe: `entregables/Alonso_Basualdo_Semana2_PFY2201.docx`.
...
- **Semana 4:** migración a Bootstrap 5 en `index.html` y `productos.html` (navbar, carrusel, cards, tema oscuro). La hoja de Semana 2 queda archivada en `css/`.
